...
async function Zippyshare(url) {
  return new Promise(async(resolve, reject) => {
    try {
      const isZippyshareUrl = (validZippyshareUrl.test(url))
      var zippy = _url.parse(url);
      if (zippy.hostname.search( 'zippyshare.com' ) < 0) return reject (new Error(`No valid a zippyshare url`));
